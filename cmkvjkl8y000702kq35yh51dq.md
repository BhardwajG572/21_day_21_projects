...
datePublished: Mon Jan 26 2026 19:09:41 GMT+0000 (Coordinated Universal Time)
cuid: cmkvjkl8y000702kq35yh51dq
...
tags: chai, git, gitcommands, hiteshchoudharylco, hiteshchaudhary, chaiaurcode, chaicode, chaicohort, chai-code, piyushgarag
...
2. No History(The “Ctrl+Z” Limit)
    ...
    We know that when we save a file and close the editor, our “Undo” history is gone. If we realized that the code we wrote last week was actually better than the code we wrote today, there is no option of going back and retrieving it. Either we have to make a backup of the file locally by putting names “back\_up\_version1\_on\_some\_date” in some backup folder, or again searching about the changes in the backup folder is very cumbersome. it may work in some small projects,but on large team or working on a complex/big project, retrieving the backup is very hard.
    
3. The “Blame Game”(Lack of Accountability).
    
    In this pendrive world, when the application crashed, nobody knew who wrote the bad code, and sometimes teammates start fighting with each other and saying that “I didn’t touch that function”, or “well, it was working fine before you took the pendrive !! “.
...
So what he did was disappear for a weekend(literally about 10 days to a working prototype) and wrote his own. He named it Git.
...
We can think of VCS not as a “backup tool”, but as a Time Machine for our project. It records changes to a file or set of files over time so that we can recall specific versions later.

### Difference between the old way (Pendrive) vs the new way (VCS/Git)
